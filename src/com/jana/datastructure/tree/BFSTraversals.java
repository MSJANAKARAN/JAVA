...
public class BFSTraversals {
	...
	private static void levelOrderRecursive(TreeNode TreeNode, int level, ArrayList<ArrayList<Integer>> res) {
		if (TreeNode == null)
			return;
//		The idea is to traverse the tree recursively, starting from the root at level 0. When a TreeNode is visited, 
...
//		are recursively processed in the same way. This effectively performs a level-order traversal using recursion.
		...
		// Add current TreeNode's data to its corresponding level
		res.get(level).add(TreeNode.data);
		// Recur for left and right children
		levelOrderRecursive(TreeNode.left, level + 1, res);
		levelOrderRecursive(TreeNode.right, level + 1, res);

	}

	private static void levelOrderQueue(TreeNode root, ArrayList<ArrayList<Integer>> res) {
//		The idea is to use a queue to traverse the tree level by level. 
//		Start by adding the root to the queue. Then, repeatedly remove a TreeNode from the queue, 
//		store its value in the result, and add its left and right children to the queue. 
//		Continue this process until the queue is empty.
//		Using queue such that,TreeNodes accessed in the order of insertion. i.e level order
	...
	private static void zigzagTraversal2Stack(TreeNode root, ArrayList<Integer> res) {
//		s1 stores TreeNodes of the current level, and s2 stores TreeNodes of the next level.
//		TreeNodes in s1 are processed from top to bottom, and their children are pushed onto s2 in left → right order.
//		TreeNodes in s2 are then processed from top to bottom, and their children are pushed onto s1 in right → left order.
...
//		Level Order Traversal technique is a method to traverse a Tree such that all TreeNodes 
...
//		For odd-numbered levels, traverse the TreeNodes from left to right and 
//		for even-numbered levels, traverse the TreeNodes from right to left.
